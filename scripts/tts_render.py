...
Two backends, selected by the channel config's `provider` field:
...
This is a two-branch dispatch, NOT a provider framework: no registry, no plugins,
no factories. Adding a third backend would mean one more `elif` and one more
small `_synthesize_*` function, and that is the intended ceiling.
...
SUPPORTED_PROVIDERS = (PROVIDER_EDGE, PROVIDER_GOOGLE)
...
def render_episode_tts(
    ...
    # Same reasoning for Google auth: resolve credentials once, before touching
    # anything, so an auth problem can never leave a half-rendered episode.
...
def _main() -> int:
    ...
    parser.add_argument("--input-mode", default="text", choices=["text", "ssml"])
